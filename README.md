...
| **A metamodel in versions** — a pack is stored per version, drafted from another, compared with another row by row, published (and then frozen, so what was validated against it stays validated) or retired. Every part of a pack — the pack, a domain, a type, a relationship type, an attribute — carries a `properties` bag the engine keeps and never interprets, and an attribute declares its own rules: a default, several values, a unit, a pattern, bounds, a group and help. A pack declares the **viewpoints** its diagrams are drawn under, frozen with the version and compared by the diff, and every relationship type names the ArchiMate relationship it is drawn as | Metamodel page (Versions tab), `ea metamodel …`, `ea viewpoints` |
...
| **Generated architecture views** — any neighbourhood, impact or answer as an architecture diagram in the notation of this repository's own architecture documents (Mermaid, stereotypes from the pack, every shape filled by its ArchiMate layer, with a chip per layer above the diagram in that layer's colour), downloadable as Markdown; or exported as a draw.io drawing through a dialogue where you name a **viewpoint** the pack declares (layered, application cooperation, process cooperation, staged delivery), the focus, the depth and the layers: the file carries a title, a band per layer, type or related element, shapes sized to their names in a readable font, nesting and spanning where the viewpoint says so, an arrowhead per ArchiMate relationship, labelled edges with a port at each end, a legend, ArchiMate stencils and the element identifier on every shape, on a page sized to the drawing, laid out the same every time, at two levels of detail: an **overview** that keeps the focus and what it reaches through structural relationships, merges parallel lines and holds at most thirty elements, or the **full** view. Shapes can be dragged to arrange a view (never saved) and the export follows that instead. Nothing is drawn by hand | Element, Impact, Ask, Target state and Metamodel pages, `ea view --viewpoint --detail`, `ea viewpoints` |
...
uv run ea viewpoints                                # what the applied metamodel says a diagram may be for
uv run ea view LDC-CURR --viewpoint application_cooperation --fmt drawio --out app.drawio
...
abstract flags, attributes, provenance, source of record and owners),
relationship types (source and target or `ANY`, inverse, qualifiers,
provenance, attributes of their own, the ArchiMate relationship each is drawn
as) and the viewpoints its diagrams are drawn under. Anything the format does not name is kept
