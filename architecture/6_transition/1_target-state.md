...
flowchart LR
  ...
  g18["⊘ The application is sized for the PoC, not for the estate [GAP18]"]:::implementation
  g1 -.-> p2
  g2 -.-> p2
  g16 -.-> p2
  g3 -.-> p3
...
| `GAP18` | **The application is sized for the PoC, not for the estate** — the store answers a traversal in milliseconds at the Requester's figure (decisions 0016 to 0018), but six services above it read every element and every relationship of the organisation into one request, a trace still builds the whole in-process graph the store made unnecessary, and nothing declares what size the application is built for, so nothing fails when it is exceeded | `PLAT1` and `PLAT3` | Initiative 16 (opened 2026-09-20, awaiting the Understanding gate): `ASM6` restated with the horizon the Requester named, one declared size the code reads and the suite holds, the traversal unwrapped from the in-process graph, and the other five whole-model reads bounded or declared deliberate |
