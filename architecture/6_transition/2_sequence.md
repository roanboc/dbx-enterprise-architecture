...
flowchart LR
  ...
  s1l["1l — export as a diagram"]:::implementation
  ...
  s1 --> s1b --> s1c --> s1d --> s1e --> s1f --> s1g --> s1h --> s1i --> s1j --> s1k --> s1l
  s1g --> s2
  s1g --> s3
  s3 --> s3b
  s2 --> s4
...
| 1l — export as a diagram (initiative 22, built 2026-09-23) | `GAP23` | `PLAT1`, extended | Step 1k | An architect exports an impact view through the application cooperation viewpoint and opens a draw.io file with a band per type, a data entity drawn inside the component that encapsulates it, an arrowhead per kind of relationship and a label on every line; the same command from the command line gives the same file, byte for byte |
...
Steps 1b to 1l all ran locally, on DuckDB. Step 1h needs no platform,
