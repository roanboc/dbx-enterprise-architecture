...
| 1l — export as a diagram (initiative 22, built 2026-09-23) | `GAP23` | `PLAT1`, extended | Step 1k | An architect exports an impact view through the application cooperation viewpoint and opens a draw.io file with a band per type, a data entity drawn inside the logical data component that encapsulates it, an arrowhead per kind of relationship and a label on every line; the same command from the command line gives the same drawing |
