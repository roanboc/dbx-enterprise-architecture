...
| `BPROC3` | **Answer an architecture question** — a question about ownership, dependencies or impact, answered at once or, when one answer would not do, as a **deep dive** | Somebody asks | The assistant runs tools over the model and composes a document with a generated view; identifiers are checked. **For a deep dive** it first settles a brief with the reader in conversation — what the analysis is about, which kind of analysis, how far it reaches and what it is for — a few questions at a time with the choices the model allows; the reader answers with a choice or in their own words; then it analyses what they settled, draws a view for each aspect of the analysis and states what it found | A cited answer with a diagram, or a deep-dive document: the brief, a summary, several views and the findings, each citing the elements it rests on |
