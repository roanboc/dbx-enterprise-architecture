...
| -- | ------- | ---------- | ------------ |
| `BSVC1` | **Architecture knowledge** — the enterprise's architecture as a queryable, cited, drawn body of knowledge: what exists, who owns it, what depends on what, what a work package changes — and the deep dives kept on it, rated by the people who read them, so an analysis builds on the last one rather than starting from a blank page | Solution architects, stewards, the data and analytics unit, and any agent | Loading (`BPROC1`), answering (`BPROC3`), analysing (`BPROC4`) |
...
| `BPROC2.2` | **Hand in a proposal** — a design page, or a drawing, becomes a change set, refined with the assistant, and a revised page updates it | A design page in a proposal template the organisation declares — its own, or one the repository ships, the ArchiMate reference among them — as text, a file or a link; or a draw.io drawing — one the application exported and a person drew on, or one drawn from nothing ([initiative 26](../scope/26_drawings-returned-and-the-model-connected.md), pending Understanding) | The assistant reads the page as its template says, identifies elements and relationships, links what exists and adopts what is new. **It settles the change top-down**: first why — the goals, drivers or capabilities the change serves — and which business processes, services and roles it changes; only then the information, application and technology rows, each of which must trace up to a business or strategy element. **It keeps a system's inside out**: an element that relates only to its own system is linked from that system — the page that describes it — rather than modelled (principle `P9`). **What it cannot settle it asks about rather than guesses** — which existing element a name means, which type a row is, which relationship joins two elements, what should happen to what depends on something being retired — a few questions at a time, the most blocking first, each with the choices the model allows. **A drawing is read by its stamp** (initiative 26): what the application drew is known by the element or relationship it stands for; a shape or a line a person added is a new element or relationship, typed from its shape by the metamodel's notation or asked about; a renamed shape is asked about as a rename; a shape taken out is asked about — out of the model, or only out of the picture — and is never a deletion unless the architect says so; a shape moved is nothing. The architect answers, picks, or says it in their own words; the assistant redrafts, and asks again until nothing is unclear. The draft is kept between sittings. The architect reads the impact of the change — what depends on what it changes, what it leaves dangling, who must review it — reviews the merge log and applies it. A revised page handed to the same branch updates what the last one wrote and lists what it no longer carries | Rows on a branch, the proposal kept with it as one revision among its others, with its impact and the conversation that settled it |
...
| `BPROC2.4` | **Review a branch** — the second person decides | A review is requested | Each reviewer reads the merge log for their types beside the proposal the rows came from, its revisions, its impact and a generated view of the change; approves or sends back with a comment; a branch is approved when every touched type has an approval from one of its reviewers | Approved, or back to draft |
...
| `BPROC3` | **Answer an architecture question** — a question about ownership, dependencies or impact, answered at once or, when one answer would not do, as a **deep dive** that is kept | Somebody asks | The assistant runs tools over the model and composes a document with a generated view; identifiers are checked. **For a deep dive** it first settles a brief with the reader in conversation — what the analysis is about, which kind of analysis, how far it reaches and what it is for — a few questions at a time with the choices the model allows; the reader answers with a choice or in their own words. It then analyses what they settled: it weighs the **maturity** of every element it rests on, says where an element and what documents it disagree, draws it from the top down — the context and the overview to be presented, then each aspect and the detail in the metamodel's notation — names the work packages in flight that change what it read, reads what the enterprise's **connected systems** say about the elements it rests on — the pages they link to, the facts the system that masters them holds — and cites them as that system's, never as the model's, with where they and the model disagree (initiative 26), states its findings, and considers the earlier deep dives on the same elements by the ratings people gave them. The deep dive is catalogued and kept, and listed on every element it cites; whoever reads it later rates it, one to five stars, one rating per person, which that person may clear | A cited answer with a diagram, or a deep dive in the catalogue — handed out as a styled PDF with its draw.io diagrams — that later analyses weigh by how it was rated |
