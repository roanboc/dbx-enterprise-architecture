...
| `ROLE5` | **Agent** — the assistant's role, held by `ACT6` | Read through the tools a reader could use; draft a proposal's change set for an architect to review; ask the architect about what the draft leaves unclear, and redraft from the answers; ask a reader what a deep dive should cover, and analyse what they settle | Write to `main`; write an approval; write anything an architect has not ticked; settle a question it asked by itself |
