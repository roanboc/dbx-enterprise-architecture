...
| `ACT6` | **Architecture assistant** — the AI agent that answers questions and reads proposals through tools, asking the architect about what a proposal leaves unclear and a reader what a deep dive should cover (component `ACMP5`, `ACMP10`) | May read everything a reader may, and what the organisation's connected systems show that reader, as that reader (initiative 26, pending Understanding); never writes an approval (principle `P3`), nor anything a connected system says |
...
| `ROLE4` | **Reader** — every signed-in user | Browse, search, open elements, run impact and target-state analyses, ask the assistant, run a deep dive and keep it in the catalogue, withdraw their own, rate any deep dive and clear their own rating, download views, documents and deep dives, download the proposal templates; read the model through an agent of their own connected to the application's tool server, which acts as them (initiative 26, pending Understanding) | Change the model; withdraw another person's deep dive |
| `ROLE5` | **Agent** — the assistant's role, held by `ACT6` | Read through the tools a reader could use; draft a proposal's change set for an architect to review; ask the architect about what the draft leaves unclear, and redraft from the answers; ask a reader what a deep dive should cover, and analyse what they settle, weighing the earlier deep dives by their ratings; read the organisation's connected systems as the person it answers, citing what they say as theirs (initiative 26, pending Understanding) | Write to `main`; write an approval; write anything an architect has not ticked; settle a question it asked by itself; rate a deep dive; write to a connected system, or take what one says for the model's |
