...
| `ROLE4` | **Reader** — every signed-in user | Browse, search, open elements, run impact and target-state analyses, ask the assistant, run a deep dive and keep it in the catalogue, withdraw their own, rate any deep dive, download views, documents and deep dives, download the proposal templates | Change the model; withdraw another person's deep dive |
