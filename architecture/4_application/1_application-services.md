...
| `ASVC6` | **Architecture views** — a neighbourhood, an impact or an answer rendered as an architecture diagram in the notation of this repository's own documents (Mermaid), every shape filled by its architecture layer and a chip per layer above the diagram in that layer's colour, copied or downloaded as Markdown; the same view exported as a draw.io drawing through the export dialogue, where the reader names a viewpoint the metamodel declares, the focus (the element the page is about, and any the reader adds), the depth and the layers, or keeps the arrangement they gave the shapes in the browser, which is never saved. The file carries a title, the bands the viewpoint defines, shapes sized to their names in a readable font, nesting and spanning where the viewpoint says so, an arrowhead per ArchiMate relationship, a labelled edge with a port at each end, a legend of the layers and relationship kinds present, ArchiMate stencils, a link on every shape and a page sized to the drawing; two exports of one view are one drawing, the file differing only in the time it says it was exported. The reader also picks the level of detail: an **overview**, the default, keeps the focus and what it reaches through structural relationships, every line touching the focus, only the structural kinds among the rest (the viewpoint may name its own), parallel lines merged into one labelled with every verb, and at most thirty elements, the least connected going first and the note saying how many; **full** draws everything. The screens keep the layout the browser gives them | Architects, the agent | Element (Graph tab), Impact, Ask, Target state and Metamodel pages; `ea view --viewpoint --layers --detail`, `ea viewpoints` |
...
| `ASVC6` | ⚙ «Application Service» Architecture views | `ASVC1` | ⚙ «Application Service» Metamodel management | constrained by | the viewpoints a reader may pick, and the arrowhead a relationship takes, come from the applied version |
