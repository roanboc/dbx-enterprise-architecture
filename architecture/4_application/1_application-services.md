...
| `ASVC16` | **Connected systems** — the enterprise's systems the assistant may read over the Model Context Protocol (initiative 26): an admin says where each answers, the read-only tools the assistant may call, the element types it masters, the page addresses it answers for, and as whom it is read — the reader's own platform identity, the organisation's credential for named roles, or nothing. Ask, a deep dive and Propose's hosted reader read them as the person asking, a bounded number of times per answer; what a system says is cited as the system's and never written into the model | Admins connect; every role that may ask reads through them | Connected systems page; `ea systems` |
