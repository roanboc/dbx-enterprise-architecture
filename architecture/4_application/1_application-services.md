...
| `ASVC14` | **Deep dives** — the analyses kept from the model (decision 0024): a deep dive is kept the moment it is written, in the organisation's own store, catalogued under the domains and element types of what it is about, its kind, the work package it concerns, the branch and the metamodel version it read, who asked and when; the catalogue lists and narrows them by any of those, by the element they cite and by rating, and opens one; whoever may read rates it — one to five stars with a line of why, one rating per person — and sees the average and how many rated it; its pack downloads again at any time; it can be run again, which keeps a new one from the same brief that names the one it came from; its author or an admin withdraws it, which keeps its row. Every element it cites lists it on its own page, and the next deep dive on the same elements weighs the earlier ones by their rating. A kept deep dive is never edited | Readers and every role that may read; the assistant reads them and never rates | Deep dives page; the Deep dives card on the Element page; the earlier deep dives beside a brief on Ask |
...
| `ASVC14` | ⚙ «Application Service» Deep dives | `ACMP6` | ▭ «Application Component» Web application | realized by | the Deep dives page, and the list on the Element page |
