...
| `ACMP7` | **Command line** — `ea` with init, load-pack, export-pack, import, export, validate, stats, `feed list/save/run/delete`, `runs list/show`, find, get, set, neighbours, trace, impact, view, target, health, sql, summary, `branch list/create/diff/review/approve/merge/abandon`, `reviewers list/set`, `metamodel versions/draft/publish/retire/delete/diff/check` and `org list/create/rename/default/apply/delete`; `--branch` (or `EA_BRANCH`), `--as` (or `EA_ROLE`) and `--org` (or `EA_ORG`) on every command | `src/ea/cli.py` | `ASVC1`–`ASVC4`, `ASVC7`, `ASVC8`, `ASVC11` | Running |
