...
| `ACMP4` | **Importer and exporter** — the CSV contract in both directions. Reads a directory of CSV files through a mapping, builds elements, relationships and links, validates against the registry, reports, loads idempotently into the current branch; derives the current state from the source's lifecycle text (mapping table, then keywords); resolves endpoints, link owners and work packages in one read each rather than one per row, and counts every issue while keeping the first two thousand (decision 0019). Writes the same three files back out, paged, elements as one wide file whose columns are every attribute any element carries — a relationship's row states the source system it belongs to, which is what makes the trip out and back land on the same edge | `src/ea/importer/csv_import.py`, `src/ea/importer/csv_export.py`, `src/ea/importer/mapping.py`, `connectors/tool-export/mapping.yaml` | `ASVC3` | Running |
