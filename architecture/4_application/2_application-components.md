...
| `ACMP14` | **Feed runner** — reads the landing tables a feed names, a page at a time, and puts them through `ACMP4` under the feed's own mapping, then empties what it loaded — after the load, so a run that stops repeats itself rather than losing rows; a feed reading a table something else maintains leaves it alone. The landing schema is the whole boundary, so the same feed runs on DuckDB and on Lakebase | `src/ea/importer/feeds.py` | `ASVC12` | Running |
