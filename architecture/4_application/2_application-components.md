...
| `ACMP2` | **Graph store** — the storage interface: schema, packs, elements, links, relationships, traces, history, read-only SQL; every read and write honours the current branch (`src/ea/backend/branching.py`), and the branch, diff, merge and proposal operations | `src/ea/backend/base.py`, `src/ea/backend/sql.py` (portable DDL and recursive trace queries), `src/ea/backend/factory.py` | `ASVC2`, `ASVC4` | Running |
| `ACMP2.1` | **DuckDB backend** — the interface on one DuckDB file; optimistic concurrency on `_version`, change log on every write (with the branch), frame-based upserts that skip rows that change nothing, the branch overlay as `UNION` views over `main` minus the overridden rows, guarded read-only SQL | `src/ea/backend/duckdb_backend.py` | `ASVC2`, `ASVC4` | Running |
