...
2026-09-19. A row marked **Pending** names the initiative that will build it.
...
| `ACMP13` | **Metamodel lifecycle and organisation services** — the versions of a pack and what becomes of one: a draft started from a stored version and edited in place, published and frozen, retired, or deleted while nobody applies it; the difference between two versions field by field; the compatibility check that validates an organisation's whole main against a candidate version before it is applied; and the organisations themselves — the current one as a context variable, the list, the default, a new one created from a copy of another's content, renamed, deleted, and given a version to apply once the check is clean | `src/ea/services/metamodel.py`, `src/ea/services/organisations.py`, `src/ea/backend/organisations.py`, `src/ea/metamodel/diff.py` | `ASVC1`, `ASVC11` | Running |
