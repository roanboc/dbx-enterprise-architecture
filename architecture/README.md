...
| 1 | Strategy | Why does this exist, and what must it be able to do? | `Local` — [1_strategy/](./1_strategy/README.md): motivation — six stakeholders, five drivers, twelve assessments, six goals and nine principles — the one value stream with the capability it realises, and a one-page brief that says the same thing to a sponsor without an identifier in it; `◐` until the Direction gate |
