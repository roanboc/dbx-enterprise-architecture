...
| 18  | [18_source-feeds-and-import-history.md](./18_source-feeds-and-import-history.md) | **not built — Understanding pending** | Source feeds and import history: a landing table per source in the platform's catalogue read through the same pipeline, a deletion indicator that retires by default and deletes only where a source is configured to, a feed that writes to a branch or to `main` as its configuration says, and a record of every run with the before-images that make one reversible. Analysed and aligned; the first thing to settle is whether the deployed application can read a catalogue table at all |
