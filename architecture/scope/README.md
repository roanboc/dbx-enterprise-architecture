...
| 16  | [16_the-size-the-model-is-built-for.md](./16_the-size-the-model-is-built-for.md) | branch `claude/metamodel-improvements-next-steps-aesadj` (opened 2026-09-20; Understanding presented, no code written) | The size the model is built for: `ASM6` restated with the horizon the Requester named rather than corrected, one declared size the application reads and the suite holds, the traversal unwrapped from the in-process graph the store made unnecessary, and the other five whole-model reads bounded or declared deliberate |
